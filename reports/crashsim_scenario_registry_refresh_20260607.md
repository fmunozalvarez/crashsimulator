...
| --- | --- |
| Scenario status | `SCENARIO_STATUS.md` now summarizes the 82-scenario registry and the newly added resilience/DG/RAC/ASM/APEX/ORDS layer. |
| End-user guide | `docs/CRASHSIMULATOR_USER_GUIDE.md` now reports 82 scenarios and the current protection/recovery helper coverage. |
| V2 notes | `README_V2.md` now reflects the 82-scenario registry, helper-script ZIP permissions, and new recovery helper coverage. |
...
| References | `docs/reference/scenario_registry_82_reference.md` was added as a sanitized scenario registry reference. |
...
The framework can list, runbook, and readiness-check all 82 scenarios. Scenario
execution is topology-aware: destructive actions are blocked when the required
PDB, Data Guard, RAC, ASM, redundancy, local filesystem, backup-piece, APEX,
ORDS, or lab object prerequisites are missing.
