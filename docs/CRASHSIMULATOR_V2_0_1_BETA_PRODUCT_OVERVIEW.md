# CrashSimulator v2.0.1 Beta Product Overview
...
Version `v2.0.1 beta` is the current V2 framework release. It is a
...
intent and typed confirmation tokens. Where a scenario is not safe or not
supported by the discovered topology, CrashSimulator reports the reason instead
of forcing execution.
...
Current product version: `v2.0.1 beta`

Version `v2.0.1 beta` includes the V2 single-script framework,
...
The current catalog contains `97` total entries:

- `82` database-host, infrastructure, application access-path, and compliance
  scenarios.
- `15` Autonomous Database cloud-service scenarios, `ADB01` through `ADB15`.
- `54` destructive host/infrastructure drills.
- `43` logical or cloud-service drills.
...
Version `v2.0.1 beta` includes report families for:
...
The MAA readiness report gives a best-effort posture assessment. It reviews
signals across:
...
The ADB family contains `ADB01` through `ADB15`. Current ADB implementation is
readiness/report driven, with future seeded logical and OCI control-plane
helpers planned.
...
Known limitations for `v2.0.1 beta`:
...
- `docs/reference/scenario_registry_97_reference.md`: current scenario catalog
  reference.
...
CrashSimulator v2.0.1 beta is best understood as an Oracle resilience practice
