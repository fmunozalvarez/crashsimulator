...
| `75` | ORDS database pool misconfiguration | Plan-only until a reversible pool-setting backup/restore flow is approved for the target ORDS version. |
...
| `80` | APEX session continuity test | Plan-only until a test APEX application/session script exists. |
